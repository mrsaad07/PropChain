...
const MaintenanceHistory = ({ refreshTrigger }) => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      setLoading(true);
      try {
        const res = await paymentAPI.getMaintenanceHistory(account, { page, limit: 5 });
  ...
  const handleExport = () => {
    const exportData = payments.map(p => ({
      ...
      Apartment: p.apartmentNo,
      Amount: p.amount,
      TxHash: p.blockchainTxHash
    }));
    downloadCSV(exportData, 'Maintenance_History.csv');
  };

  const handlePrev = () => setPage(p => Math.max(1, p - 1));
  const handleNext = () => setPage(p => Math.min(totalPages, p + 1));

  if (loading && page === 1) return <div className="text-center py-4 text-gray-500 text-sm">Loading history...</div>;
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
      ...
      {payments.length > 0 ? (
              ...
              <tbody className="divide-y divide-gray-50">
                {payments.map((p) => (
                  <tr key={p._id} className="text-sm hover:bg-gray-50">
                    ...
                    <td className="py-3 text-gray-600 truncate max-w-[150px]">{p.buildingId}</td>
        ...
        <p className="text-gray-500 text-sm py-4">No payment history found.</p>
      )}
    </div>
  );
};
